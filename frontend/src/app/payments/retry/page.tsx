...
function RetryContent() {
  ...
  const retry = async () => {
    ...
    if (!clientKey) return setError('토스페이먼츠 클라이언트 키가 설정되지 않았습니다.');
    if (!clientKey.startsWith('test_ck_') && !clientKey.startsWith('live_ck_')) {
      return setError('토스페이먼츠 API 개별 연동 클라이언트 키(ck)를 설정해주세요.');
    }
    try {
      ...
      await payment.requestPayment({
        method: 'CARD', amount: { currency: 'KRW', value: prepared.amount },
  ...
  return <main className="max-w-xl mx-auto px-4 py-24 text-center">
    ...
    <button onClick={retry} disabled={loading} className="bg-black text-white px-6 py-3 disabled:opacity-50">{loading ? '결제창 준비 중...' : '테스트 카드 결제창 열기'}</button>
