...
TossPayments SDK v2의 API 개별 연동 방식을 사용합니다. `NEXT_PUBLIC_TOSS_CLIENT_KEY`에 `test_ck`로 시작하는 클라이언트 키를, `TOSS_SECRET_KEY`에 같은 테스트 상점의 `test_sk`로 시작하는 시크릿 키를 주입합니다. 키 값은 소스코드와 `.env.example`에 포함하지 않습니다. 프런트에서는 `payment()`으로 CARD 통합결제창을 호출하고, 서버 승인 API가 성공한 후에만 결제와 주문을 완료 처리합니다.
