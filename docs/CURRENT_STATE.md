...
이 문서는 v0.1.1 ~ v0.1.9 완료 상태, v0.2.0 ~ v0.2.9 운영 고도화, v0.3.0 ~ v0.3.9 상품/전시/운영관리 고도화, v0.4.0 ~ v0.4.9 인사관리/권한관리/감사 로그/사업자 설정 저장 및 릴리스 검증 결과, v0.5.0 ~ v0.5.9 재고/생산/바코드 구현과 검증 결과, v0.6.0 ~ v0.6.8 유통관리/송장/배송 구현과 UI 검증 결과를 함께 기록한다.
...
| v0.6 유통관리/송장/배송 | 진행. 출고 지시, 택배사/배송 방법, 송장번호 수동 저장/자동 생성/수정, 송장 라벨 HTML 미리보기와 출력 이력, 배송 추적 이벤트, 반품 배송 정보, 바코드 출고 검수와 관리자 UI 검증을 완료했고 v0.6.9에서 릴리스 검증과 v0.7 이관을 진행 |
