...
이 문서는 v0.1.1 ~ v0.1.9 완료 상태, v0.2.0 계획 수립 기준, v0.2.1 인증/세션/보안 안정화, v0.2.2 결제/환불, v0.2.3 미디어 업로드, v0.2.4 리뷰 운영/감사 로그, v0.2.5 DB 마이그레이션/스키마 정리, v0.2.6 CI/환경 분리, v0.2.7 알림/관리자 권한, v0.2.8 운영 분석 기반, v0.2.9 릴리스 검증, v0.3.0 상품/전시/운영관리 계획, v0.3.1 상품 마스터 고도화, v0.3.2 상품 상세페이지 블록 에디터, v0.3.3 카테고리/상단 네비 관리자 관리, v0.3.3.1 프론트 한글화 hotfix, v0.3.4 메인 배너 CMS, v0.3.5 상품 판매 상태 운영, v0.3.5.1/v0.3.5.2 관리자 네비게이션 foundation/hotfix, v0.3.6 상품 운영 UX/대량 관리, v0.3.7 사용자 전시 화면 반영 결과를 함께 기록한다.
...
- 직원 생성/수정 화면과 API, 권한 그룹/권한 매트릭스, permission 기반 사이드바 전환.
- 재고관리, 생산 입고, 바코드 자동화.
