...
- v1.0.1에서 문서 인덱스, 프로젝트 요약, 기능명세, AI 전략 문서의 오래된 기준 버전과 예정 표현을 정리했다.
- v1.0.2에서 포트폴리오 데모 데이터/계정 가이드를 추가했다.
- v1.0.3에서 backend test, frontend lint, frontend build 재검증과 smoke test 결과 문서를 정리했다.
- v1.0.4에서 v1.0.0 ~ v1.0.3 태그/체크리스트를 확인하고 최종 포트폴리오 릴리스 태그를 생성한다.
