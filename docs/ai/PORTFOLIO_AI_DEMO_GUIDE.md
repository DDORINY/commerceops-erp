...
v0.9.1 ~ v0.9.7에서 관리자 AI 운영 화면 연결이 완료되었으므로, 발표나 면접 시연은 [AI 운영 화면 포트폴리오 데모 시나리오](./AI_OPERATIONS_DEMO_SCRIPT.md)를 기준으로 진행한다.

시연 순서는 `/admin/ai` 개요에서 시작해 상품 추천, 수요 예측, 리뷰 분석, 이상 주문 탐지, 리스크 알림, AI 리포트 순서로 이어진다. 각 화면의 점수는 자동 의사결정이 아니라 관리자 검토를 돕는 참고 지표로 설명한다.
