...
- 세부 규칙은 [버전 작업 규칙](../operations/VERSION_WORKFLOW.md)을 따른다.
...
- [x] `docs/architecture/DATABASE_SCHEMA.md`를 실제 DDL 기준으로 갱신
- [x] `docs/architecture/DB_MIGRATION.md` 운영 기준 추가
...
- [x] v0.3 체크리스트 존재 확인
- [x] `docs/plans/V0.4_HR_PERMISSION_PLAN.md` 확인 및 보강
...
상세 계획은 `docs/plans/V0.5_INVENTORY_BARCODE_PLAN.md`에 기록한다.
...
- [x] 문서 작업만 수행하여 frontend lint/build와 backend test 생략
- [x] main 병합 및 `v0.5.0` 태그 생성

### v0.5.1 체크리스트

목적: SKU/바코드 재고 마스터를 구현해 상품 마스터 코드와 재고 운영 단위 코드를 분리한다.

- [x] `v0.5.1-sku-barcode-inventory-master` 브랜치 생성
- [x] `skus` 테이블과 SKU/바코드 permission seed migration 추가
- [x] 관리자 SKU 조회/생성/수정/활성 변경/바코드 재발급 API 구현
- [x] `/admin/skus` 관리자 화면과 메뉴 추가
- [x] SKU 생성/수정/활성 변경/바코드 재발급 audit log 기록
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.1` 태그 생성

### v0.5.2 체크리스트

목적: 생산 주문 생성/관리와 생산 완료 시 SKU/상품/창고 재고 자동 증가 흐름을 구현한다.

- [x] `v0.5.2-production-receipt-flow` 브랜치 생성
- [x] 생산 주문/품목/입고 Entity와 Flyway migration 추가
- [x] 생산 주문 목록/상세/생성/수정/시작/완료/취소 API 구현
- [x] 생산 완료 시 상품 재고, 창고 재고, 재고 로그, 생산 입고 이력 생성
- [x] `PRODUCTION_MANAGE` permission과 `/admin/production` 메뉴 추가
- [x] `/admin/production` 관리자 화면 추가
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.2` 태그 생성

### v0.5.3 체크리스트

목적: SKU 바코드를 기준으로 라벨 미리보기와 출력 이력 기록 기반을 구현한다.

- [x] `v0.5.3-barcode-label-printing` 브랜치 생성
- [x] v0.5.2 완료 상태와 태그 확인
- [x] `barcode_labels` 테이블과 `/admin/barcodes` 메뉴 seed migration 추가
- [x] 바코드/SKU 검색, 바코드 단건 조회, 라벨 생성, 출력 이력 기록 API 구현
- [x] `INVENTORY_READ`, `BARCODE_MANAGE` permission 검증 적용
- [x] 바코드 라벨 생성/출력 audit log 기록
- [x] `/admin/barcodes` 관리자 화면 추가
- [x] 관련 문서와 체크리스트 갱신
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.3` 태그 생성

### v0.5.4 체크리스트

목적: 바코드 기준으로 SKU/상품/창고 재고를 조회하고 간단 입고/출고를 처리한다.

- [x] `v0.5.4-barcode-stock-processing` 브랜치 생성
- [x] v0.5.3 완료 상태와 태그 확인
- [x] 바코드 재고 조회 API 구현
- [x] 바코드 입고/출고 API 구현
- [x] 기존 `WarehouseStock`, `InventoryLog` 재사용 기준 정리
- [x] 바코드 입고/출고 audit log 기록
- [x] `/admin/barcode-stock` 관리자 화면과 메뉴 추가
- [x] 관련 문서와 체크리스트 갱신
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.4` 태그 생성

### v0.5.5 체크리스트

목적: 창고별 재고 실사 세션, 실사 수량 입력, 차이 조정 흐름을 구현한다.

- [x] `v0.5.5-stock-count-adjustment` 브랜치 생성
- [x] v0.5.4 완료 상태와 태그 확인
- [x] `stock_count_sessions`, `stock_count_items` 테이블 migration 추가
- [x] 재고 실사 세션/품목 Entity, Repository, Service, Controller 구현
- [x] 실사 품목의 시스템 수량 snapshot, 실사 수량, 차이 수량 계산 구현
- [x] 실사 완료 시 차이 수량을 상품/창고 재고와 `InventoryLog(ADJUST)`에 반영
- [x] `STOCK_COUNT_MANAGE` permission과 `/admin/stock-counts` 메뉴 추가
- [x] `/admin/stock-counts` 관리자 화면 추가
- [x] 관련 문서와 체크리스트 갱신
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.5` 태그 생성

### v0.5.6 체크리스트

목적: 창고 내부 위치/로케이션 관리와 위치별 SKU 재고 기반을 구현한다.

- [x] `v0.5.6-warehouse-location-management` 브랜치 생성
- [x] v0.5.5 완료 상태와 태그 확인
- [x] `warehouse_locations`, `warehouse_location_stocks` 테이블 migration 추가
- [x] 창고 위치 Entity, Repository, Service, Controller 구현
- [x] 위치별 SKU 재고 조회 기반 구현
- [x] 창고별 위치 코드 중복 검증 구현
- [x] 창고 위치 생성/수정/활성 상태 변경 audit log 기록
- [x] `/admin/warehouse-locations` 관리자 화면과 메뉴 추가
- [x] 관련 문서와 체크리스트 갱신
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.6` 태그 생성

### v0.5.7 체크리스트

목적: 안전재고 기준과 재고 부족 알림 조회 기반을 구현한다.

- [x] `v0.5.7-inventory-alert-safety-stock` 브랜치 생성
- [x] v0.5.6 완료 상태와 태그 확인
- [x] `inventory_alert_rules` 테이블 migration 추가
- [x] 안전재고 기준 Entity, Repository, Service, Controller 구현
- [x] 기준 목록/생성/수정/활성 상태 변경 API 구현
- [x] 재고 부족 항목 조회 API 구현
- [x] `/admin/inventory-alerts` 관리자 화면과 메뉴 추가
- [x] 관련 문서와 체크리스트 갱신
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.7` 태그 생성

### v0.5.8 체크리스트

목적: v0.5 재고/생산/바코드 관리자 화면과 핵심 흐름을 검증하고 남은 UI/UX 이슈를 정리한다.

- [x] `v0.5.8-inventory-production-barcode-ui-verification` 브랜치 생성
- [x] v0.5.0 ~ v0.5.7 태그와 체크리스트 확인
- [x] SKU/바코드/바코드 입출고/생산/재고/실사/창고/창고 위치/안전재고 화면 파일 확인
- [x] API/DB/프론트 문서 정합성 확인
- [x] 권한별 수동 확인 후보 정리
- [x] 관련 문서와 체크리스트 갱신
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.8` 태그 생성

### v0.5.9 체크리스트

목적: v0.5 재고/생산/바코드 범위를 통합 검증하고 v0.6 유통관리/송장/배송으로 이관한다.

- [x] `v0.5.9-inventory-release-verification-v06-handover` 브랜치 생성
- [x] v0.5.0 ~ v0.5.8 태그와 체크리스트 확인
- [x] SKU/바코드/생산/바코드 라벨/바코드 입출고/재고 실사/창고 위치/안전재고 구현 범위 확인
- [x] 권한, 관리자 메뉴, audit log 반영 범위 확인
- [x] API/DB/프론트 문서 정합성 확인
- [x] v0.5 남은 이슈와 v0.6 이관 범위 정리
- [x] `docs/plans/V0.6_DISTRIBUTION_SHIPPING_PLAN.md` 생성
- [x] frontend lint/build 및 backend test 검증
- [x] main 병합 및 `v0.5.9` 태그 생성

## v0.6 계획

목적: v0.5 재고/생산/바코드 기반 위에 출고, 택배사, 송장번호, 배송 상태 추적 중심의 유통관리 흐름을 구현한다.
...
상세 계획은 `docs/plans/V0.6_DISTRIBUTION_SHIPPING_PLAN.md`에 기록한다.

### v0.6.0 체크리스트

목적: 유통관리/송장/배송 자동화의 전체 범위와 제외 범위, DB/API/프론트/권한/감사 로그 기준, v0.7 회계/정산 이관 후보를 확정한다.

- [x] main 최신 상태 확인
- [x] `v0.6.0-distribution-shipping-plan` 브랜치 생성
- [x] v0.5.9 완료 상태와 태그 확인
- [x] 기존 주문/배송/재고/SKU/권한/감사 로그 구조 확인
- [x] v0.5에서 이관된 이슈 정리
- [x] v0.6 전체 범위와 제외 범위 확정
- [x] v0.6.0 ~ v0.6.9 작업 순서 확정
- [x] DB/API/프론트 화면 후보 정리
- [x] 권한/감사 로그 기준 정리
- [x] v0.7 회계/정산 이관 후보 정리
- [x] 문서 작업만 수행하여 frontend lint/build와 backend test 생략
- [x] main 병합 및 `v0.6.0` 태그 생성

### v0.6.1 체크리스트

목적: 주문 기반 출고 지시와 출고 품목 모델을 구현하고 관리자 출고 관리 화면을 제공한다.

- [x] main 최신 상태 확인
- [x] `v0.6.1-outbound-order-model` 브랜치 생성
- [x] 기존 주문/주문 품목/SKU/창고/권한/감사 로그 구조 확인
- [x] `outbound_orders`, `outbound_order_items` Flyway 마이그레이션 추가
- [x] `OutboundOrder`, `OutboundOrderItem`, `OutboundOrderStatus` 모델 추가
- [x] 관리자 출고 지시 목록/상세/생성/수정/피킹/취소 API 추가
- [x] `OUTBOUND_READ`, `OUTBOUND_MANAGE` permission code와 관리자 메뉴 seed 추가
- [x] 출고 생성/수정/피킹/취소 audit log 기록
- [x] `/admin/outbound-orders` 관리자 화면과 `outboundOrderService` 추가
- [x] API/DB/백엔드/프론트/현재 상태/v0.6 계획 문서 갱신
- [x] frontend lint/build 검증
- [x] backend test 검증
- [x] main 병합 완료
- [x] `v0.6.1` 태그 생성
...
목적: 택배사와 배송 방법 master 데이터를 관리하고 후속 송장/배송 흐름에서 선택할 수 있는 기반을 만든다.

- [x] main 최신 상태 확인
- [x] `v0.6.2-carrier-shipping-methods` 브랜치 생성
- [x] 기존 배송/권한/감사 로그/관리자 메뉴 구조 확인
- [x] `carriers`, `shipping_methods` Flyway 마이그레이션 추가
- [x] 택배사 목록/생성/수정/활성 상태 변경 API 추가
- [x] 배송 방법 목록/생성/수정/활성 상태 변경 API 추가
- [x] `CARRIER_MANAGE` permission code와 관리자 메뉴 seed 추가
- [x] 택배사/배송 방법 audit log 기록
- [x] `/admin/carriers`, `/admin/shipping-methods` 관리자 화면 추가
- [x] API/DB/백엔드/프론트/현재 상태/v0.6 계획 문서 갱신
- [x] frontend lint/build 검증
- [x] backend test 검증
- [x] main 병합 및 `v0.6.2` 태그 생성
...
- [x] 기존 배송/송장/권한/감사 로그/관리자 배송 화면 구조 확인
- [x] `shipments` 송장 발급 방식/발급 시각 컬럼 확장 마이그레이션 추가
- [x] `TrackingNumberSource` enum 추가
- [x] `SHIPMENT_READ`, `SHIPMENT_MANAGE` permission code와 기본 그룹 권한 seed 추가
- [x] 송장번호 수동 입력/수정 API 정리
- [x] 내부 테스트용 송장번호 자동 생성 API 추가
- [x] 송장번호 생성/수정 audit log 기록
- [x] READY 최초 송장 등록과 IN_TRANSIT 송장 수정의 재고 차감 기준 분리
- [x] `/admin/shipments` 송장번호 수동 입력/자동 생성/수정 UI 반영
- [x] API/DB/백엔드/프론트/현재 상태/v0.6 계획 문서 갱신
- [x] frontend lint/build 검증
- [x] backend test 검증
- [x] main 병합 및 `v0.6.3` 태그 생성
...
- [x] 송장 라벨 생성/조회/출력 이력 API 추가
- [x] `SHIPPING_LABEL_PRINT` permission code와 기본 그룹 권한 seed 추가
- [x] 송장 라벨 생성/출력 audit log 기록
- [x] `/admin/shipments` 송장 라벨 미리보기/출력 이력 UI 반영
- [x] API/DB/백엔드/프론트/현재 상태/v0.6 계획 문서 갱신
- [x] frontend lint/build 검증
- [x] backend test 검증
- [x] main 병합 및 `v0.6.4` 태그 생성
...
- [x] 관리자 배송 추적 이벤트 조회/등록 API 추가
- [x] 관리자 배송 상태 변경 API 추가
- [x] 배송 상태 변경/추적 이벤트 audit log 기록
- [x] `/admin/shipments` 배송 추적 이력 UI 반영
- [x] API/DB/백엔드/프론트/현재 상태/v0.6 계획 문서 갱신
- [x] frontend lint/build 검증
- [x] backend test 검증
- [x] main 병합 및 `v0.6.5` 태그 생성

### v0.6.6 체크리스트

목적: 반품 관리 흐름에 회수 송장, 수거 상태, 반품 배송비 정보를 연결한다.
...
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.6.6` 태그 생성

### v0.6.7 체크리스트

목적: 출고 지시 품목을 SKU/바코드 기준으로 검수하고 스캔 이력을 저장한다.
...
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.6.9` 태그 생성

## v0.7 계획

목적: v0.6 유통관리/송장/배송 기반 위에 매입, 매출, 환불, 배송비, 택배비, 정산, 마감, 회계 리포트 흐름을 고도화한다. v0.7.0은 계획 수립 버전이며 기능 코드와 DB migration은 작성하지 않는다.

- `v0.7.0`: 회계관리 고도화 계획
- `v0.7.1`: 회계 원장/거래 유형 모델 완료
- `v0.7.2`: 주문 매출 인식 기준 완료
- `v0.7.3`: 환불/반품 배송비 회계 처리
- `v0.7.4`: 택배비 매입/배송비 정산
- `v0.7.5`: 정산 배치/마감 기반
- `v0.7.6`: 회계 리포트 UI
- `v0.7.7`: 권한/감사 로그 연동
- `v0.7.8`: 회계 검증 데이터 정합성
- `v0.7.9`: v0.7 릴리스 검증 및 v0.8 AI 데이터셋 이관

상세 계획은 `docs/plans/V0.7_ACCOUNTING_SETTLEMENT_PLAN.md`에 기록한다.

### v0.7.0 체크리스트

목적: 회계관리 고도화의 전체 범위와 제외 범위, DB/API/UI/권한/감사 로그 기준, v0.8 AI 데이터셋 이관 후보를 확정한다.

- [x] main 최신 상태 확인
- [x] `v0.7.0-accounting-settlement-plan` 브랜치 생성
- [x] v0.6.9 완료 상태와 태그 확인
- [x] 기존 회계/매출/주문/결제/환불/반품/배송/출고/권한/감사 로그 구조 확인
- [x] v0.7 전체 범위와 제외 범위 확정
...
- [x] 문서 작업만 수행하여 frontend lint/build와 backend test 생략
- [x] main 병합 및 `v0.7.0` 태그 생성

### v0.7.1 체크리스트

목적: 회계 원장, 회계 거래 유형, 참조 도메인 연결 기준을 구현한다.

- [x] main 최신 상태 확인
- [x] `v0.7.1-accounting-ledger-transaction-model` 브랜치 생성
- [x] `accounting_ledgers`, `accounting_transactions` Flyway 마이그레이션 추가
- [x] 원장/거래 Entity, Repository, DTO, enum 추가
- [x] 원장 목록/상세 조회 API 추가
- [x] 거래 목록/상세 조회 API 추가
- [x] 관리자 회계 화면에 원장/거래 조회 섹션 추가
- [x] `ACCOUNTING_READ`, `ACCOUNTING_MANAGE` 권한 기준 반영
- [x] API/DB/백엔드/프론트/권한/현재 상태/v0.7 계획 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.7.1` 태그 생성

### v0.7.2 체크리스트

목적: 결제/출고/배송 상태 기반 주문 매출 인식 기준을 구현한다.

- [x] main 최신 상태 확인
- [x] `v0.7.2-order-revenue-recognition` 브랜치 생성
- [x] 결제 완료 시 주문 매출 `SALES` 거래 생성
- [x] 주문별 매출 인식 중복 방지 구현
- [x] 주문 매출 인식 조회/실행 API 추가
- [x] 매출 인식 이벤트 목록 API 추가
- [x] 회계 audit log 기록 보강
- [x] 프론트 accounting service 타입과 호출 메서드 추가
- [x] API/백엔드/프론트/현재 상태/v0.7 계획 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.7.2` 태그 생성

### v0.7.3 체크리스트

목적: 환불과 반품 배송비 고객/회사 부담 기준을 회계 거래로 반영한다.

- [x] main 최신 상태 확인
- [x] `v0.7.3-refund-return-fee-accounting` 브랜치 생성
- [x] 결제 환불 `REFUND` 거래 생성
- [x] 반품 승인 환불 `REFUND` 거래 생성
- [x] 반품 배송비 `RETURN_FEE` 거래 생성
- [x] `RETURN_FEE_MANAGE` 권한 추가
- [x] 환불/반품 배송비 수동 회계 반영 API 추가
- [x] 환불/반품 배송비 이벤트 목록 API 추가
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.7.3` 태그 생성

### v0.7.4 체크리스트

목적: 택배사/배송 방법 기준 내부 택배비 매입 비용과 배송비 정산 후보를 구현한다.

- [x] main 최신 상태 확인
- [x] `v0.7.4-shipping-cost-settlement` 브랜치 생성
- [x] `shipping_cost_entries` Flyway migration 추가
- [x] 배송비 비용 Entity/Repository/DTO 추가
- [x] 배송중/배송완료 시 `SHIPPING_COST` 거래 생성
- [x] 배송비 비용 조회/수동 반영 API 추가
- [x] `SHIPPING_COST_MANAGE` 권한 추가
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.7.4` 태그 생성

### v0.7.5 체크리스트

목적: 기간별 정산 배치와 마감 상태 기반을 구현한다.

- [x] main 최신 상태 확인
- [x] `v0.7.5-settlement-batch-closing` 브랜치 생성
...
- [x] 정산 배치 마감 API 추가
- [x] 중복 생성/중복 마감 방지
- [x] `SETTLEMENT_MANAGE` 권한 추가
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.7.5` 태그 생성

### v0.7.6 체크리스트

목적: 매출, 환불, 배송비, 정산 요약과 내역을 관리자 회계 화면에 표시한다.

- [x] main 최신 상태 확인
- [x] `v0.7.6-accounting-report-ui` 브랜치 생성
- [x] 관리자 회계 화면 한글 문구 정리
- [x] 회계 원장/거래/택배비 비용/정산 배치 섹션 추가
- [x] 로딩/에러/빈 상태 유지
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] backend test 검증
- [x] main 병합 및 `v0.7.6` 태그 생성
...
목적: 회계/정산 관리자 API의 세부 권한과 감사 로그 기록 기준을 정리하고, 권한/메뉴 라벨을 정상 한국어로 보정한다.

- [x] main 최신 상태 확인
- [x] `v0.7.7-accounting-permission-audit` 브랜치 생성
- [x] 회계 API 권한 코드 점검
- [x] 회계 실행 작업 감사 로그 기록 기준 점검
- [x] 권한 실패 감사 로그 흐름 점검
- [x] 회계 권한/메뉴 라벨 보정 migration 추가
- [x] 관련 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
...
v0.8의 목적은 CommerceOps ERP의 운영 데이터를 AI 학습 데이터셋으로 정리하고, PyTorch 기반 `.pt` 모델 학습 구조를 구축하는 것이다.

- `v0.8.0`: AI 데이터셋/학습 계획 수립
- `v0.8.1`: 데이터셋 추출 기반
- `v0.8.2`: 개인정보 마스킹/비식별화
...
- `v0.8.8`: 모델 평가/리포트
- `v0.8.9`: v0.8 통합 검증 및 v0.9 AI 운영 기능 이관

상세 계획은 `docs/plans/V0.8_AI_DATASET_PLAN.md`에 기록한다.

### v0.8.0 체크리스트

목적: v0.8 AI 데이터셋/학습 기반 구축의 범위, 제외 범위, 데이터 도메인, 보안 기준, 세부 작업 순서를 확정한다.
...
- [x] v0.9 이관 후보 정리
- [x] 관련 문서 갱신
- [x] main 병합 및 `v0.8.0` 태그 생성

### v0.8.1 체크리스트

목적: AI 학습용 데이터셋 후보를 관리자/배치 흐름에서 조회할 수 있는 최소 export API 기반을 만든다.

- [x] main 최신 상태 확인
- [x] `v0.8.1-ai-dataset-export-foundation` 브랜치 생성
- [x] AI 데이터셋 카탈로그 API 추가
- [x] AI 데이터셋 샘플 export API 추가
- [x] 상품/주문/리뷰/회계 거래 데이터셋 후보 지원
- [x] `AI_DATASET_EXPORT` 권한 추가
- [x] 관련 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.1` 태그 생성

### v0.8.2 체크리스트

목적: AI 데이터셋 export 과정에서 개인정보와 민감 정보가 그대로 노출되지 않도록 기본 마스킹/비식별화 기반을 마련한다.

- [x] main 최신 상태 확인
- [x] `v0.8.2-ai-dataset-privacy-masking` 브랜치 생성
- [x] 개인정보 마스킹 서비스 추가
- [x] 리뷰 본문 export 마스킹 적용
- [x] export 응답에 `privacyMasked` 플래그 추가
- [x] 관련 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.2` 태그 생성

### v0.8.3 체크리스트

목적: 상품 속성과 리뷰 데이터를 결합한 학습 데이터셋을 추가한다.
...
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.3` 태그 생성

### v0.8.4 체크리스트

목적: 주문 데이터를 수요 예측 학습에 사용할 수 있도록 날짜/시간/금액/상태 피처를 포함한 dataset key를 추가한다.
...
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.4` 태그 생성

### v0.8.5 체크리스트

목적: SKU/재고와 배송 상태/리드타임 데이터를 AI 학습 후보로 export할 수 있게 한다.
...
- [x] 재고 피처 export 구현
- [x] 배송 리드타임 피처 export 구현
- [x] 관련 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.5` 태그 생성

### v0.8.6 체크리스트

목적: 회계 거래, 정산 배치, 회계 정합성 이슈를 AI 학습 후보로 export할 수 있게 한다.
...
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.6` 태그 생성

### v0.8.7 체크리스트

목적: AI 데이터셋 export 결과를 입력으로 사용하는 `.pt` 모델 학습 파이프라인의 최소 기반을 만든다.

- [x] main 최신 상태 확인
- [x] `v0.8.7-pt-model-training-pipeline` 브랜치 생성
- [x] `ai/` 학습 디렉터리 구조 생성
- [x] baseline 학습 설정 파일 추가
- [x] baseline 학습 스크립트 추가
- [x] 모델 산출물과 export 파일 Git 제외
- [x] 관련 문서 갱신
- [x] Python 학습 스크립트 검증
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.7` 태그 생성

### v0.8.8 체크리스트

목적: 학습된 baseline 모델을 검증 데이터셋으로 평가하고 JSON/Markdown 리포트 산출 기준을 만든다.
...
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] main 병합 및 `v0.8.8` 태그 생성

### v0.8.9 체크리스트

목적: v0.8.0 ~ v0.8.8 결과를 통합 검증하고 v0.9 AI 운영 기능으로 이관할 범위를 확정한다.

- [x] main 최신 상태 확인
- [x] `v0.8.9-ai-dataset-release-verification` 브랜치 생성
- [x] v0.8.0 ~ v0.8.8 태그 확인
- [x] v0.8.0 ~ v0.8.8 체크리스트 확인
- [x] AI 데이터셋/export/마스킹/학습/평가 구조 검증
- [x] v0.9 AI 운영 기능 계획 문서 생성
- [x] 관련 문서 갱신
- [x] backend test 검증
- [x] frontend lint 검증
- [x] frontend build 검증
- [x] Python 스크립트 도움말 검증
- [x] main 병합 및 `v0.8.9` 태그 생성

### v0.8.10 체크리스트

목적: 실제 운영 데이터가 없는 포트폴리오 환경에서도 AI 학습/평가 파이프라인을 재현할 수 있도록 합성 샘플 데이터와 데모 config를 추가한다.

- [x] main 최신 상태 확인
- [x] `v0.8.10-portfolio-ai-demo-data` 브랜치 생성
- [x] 수요 예측 합성 데이터 추가
- [x] 리뷰 감성 합성 데이터 추가
- [x] 데모 학습/평가 config 추가
- [x] 학습/평가 스크립트 한글 메시지 정상화
- [x] 포트폴리오 AI 데모 실행 가이드 추가
- [x] Python 데모 학습/평가 검증
- [x] `git diff --check`
- [x] main 병합 및 `v0.8.10` 태그 생성

## v0.9 버전 계획

v0.9의 목적은 v0.8에서 만든 AI 데이터셋, 합성 샘플 데이터, baseline `.pt` 모델, 평가 리포트 기반을 관리자 운영 화면에 연결하고 최종 포트폴리오 릴리스로 정리하는 것이다.

상세 계획은 `docs/plans/V0.9_AI_OPERATIONS_PLAN.md`를 기준으로 한다.

| 버전 | 브랜치 예시 | 목적 | 완료 조건 |
| --- | --- | --- | --- |
| `v0.9.0` | `v0.9.0-ai-operations-plan` | AI 운영 기능 계획 수립 | v0.9.1 ~ v0.9.9 작업 순서와 제외 범위 문서화 |
| `v0.9.1` | `v0.9.1-ai-operations-foundation` | AI 운영 공통 기반 | 공통 DTO/API/UI와 관리자 AI 메뉴 기반 생성 |
| `v0.9.2` | `v0.9.2-ai-product-recommendations` | 상품 추천 후보 API/UI | 관리자 추천 후보 화면 표시 |
| `v0.9.3` | `v0.9.3-ai-demand-forecast` | 수요 예측 운영 화면 | 상품별 예상 수요와 근거 표시 |
| `v0.9.4` | `v0.9.4-ai-review-analysis` | 리뷰 분석 운영 화면 | 리뷰 감성/키워드/숨김 검토 후보 표시 |
| `v0.9.5` | `v0.9.5-ai-order-anomaly` | 이상 주문 탐지 | 이상 주문 후보와 사유 표시 |
| `v0.9.6` | `v0.9.6-ai-risk-alerts` | 재고/정산 리스크 알림 | 재고/정산 리스크 후보 표시 |
| `v0.9.7` | `v0.9.7-ai-report-explainability` | AI 리포트/근거 설명 | 결과별 근거 피처와 해석 패널 제공 |
| `v0.9.8` | `v0.9.8-portfolio-demo-polish` | 포트폴리오 데모 정리 | 데모 데이터, 실행 방법, 발표 시나리오 정리 |
| `v0.9.9` | `v0.9.9-final-release-verification` | 최종 통합 검증/릴리스 | v0.1 ~ v0.9 전체 검증과 최종 태그 생성 |

### v0.9.0 체크리스트

목적: AI 운영 기능의 전체 범위, 제외 범위, 세부 작업 순서, 공통 API/UI 설계 기준을 확정한다.

- [x] main 최신 상태 확인
- [x] `v0.9.0-ai-operations-plan` 브랜치 생성
- [x] v0.8.10 완료 상태 확인
- [x] v0.9 계획 문서 정상 한국어 재작성
- [x] v0.9.1 ~ v0.9.9 작업 순서 확정
...
목적: 상품별 재고와 안전재고를 기반으로 관리자용 AI 수요 예측 후보와 재고 소진 위험을 표시한다.
...
- [x] `/admin/ai/demand-forecast` 화면 추가
...
- [x] frontend build
- [x] `git diff --check`
- [x] main 병합 및 `v0.9.7` 태그 생성

### v0.9.8 체크리스트

목적: v0.9 AI 운영 기능을 포트폴리오 발표와 면접에서 설명할 수 있도록 데모 실행 흐름, 관리자 화면 시연 순서, `.pt` 모델 재현 기준을 정리한다.

- [x] main 최신 상태 확인
- [x] `v0.9.8-portfolio-demo-polish` 브랜치 생성
- [x] v0.9.0 ~ v0.9.7 완료 상태 확인
- [x] AI 운영 화면 데모 시나리오 문서 추가
- [x] 포트폴리오 AI 데모 실행 가이드 연결
- [x] CURRENT_STATE 갱신
- [x] V0.9_AI_OPERATIONS_PLAN 갱신
- [x] 체크리스트 생성
- [x] 코드 변경 없음 확인
- [x] `git diff --check`
- [ ] main 병합 및 `v0.9.8` 태그 생성
